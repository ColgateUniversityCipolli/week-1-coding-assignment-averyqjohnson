...
\hlkwd{canvas_collatz}\hldef{(}\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"blackwhite"}\hldef{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\begin{alltt}
\hlkwd{set.seed}\hldef{(}\hlnum{1313}\hldef{)}
\hlkwd{canvas_collatz}\hldef{(}\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"vrolik4"}\hldef{),} \hlkwc{background}\hldef{=}\hlsng{"#dbdbdb"}\hldef{,} \hlkwc{n}\hldef{=}\hlnum{72}\hldef{,} \hlkwc{angle.even}\hldef{=}\hlnum{0.0145}\hldef{,} \hlkwc{angle.odd}\hldef{=}\hlopt{-}\hlnum{0.05}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
%Code to insert a figure [H]ere
...
\begin{alltt}
\hlkwd{set.seed}\hldef{(}\hlnum{1313}\hldef{)}
\hlkwd{canvas_collatz}\hldef{(}\hlkwc{colors} \hldef{=} \hlkwd{colorPalette}\hldef{(}\hlsng{"blossom"}\hldef{),} \hlkwc{background}\hldef{=}\hlsng{"#1e1e2f"}\hldef{,} \hlkwc{n}\hldef{=}\hlnum{300}\hldef{,} \hlkwc{angle.even}\hldef{=}\hlnum{0.15}\hldef{,} \hlkwc{angle.odd}\hldef{=}\hlopt{-}\hlnum{0.1}\hldef{)}
\end{alltt}
\end{kframe}
\end{knitrout}
%Code to insert a figure [H]ere
...
% Numeric vector from 0 to 10
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{num.vec} \hlkwb{<-} \hlnum{0}\hlopt{:}\hlnum{10}
\hldef{num.vec}
\end{alltt}
\begin{verbatim}
...
\textbf{Solution:}
% Compute the expression for each value in num.vec

\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{results} \hlkwb{<-} \hlnum{2}\hlopt{^}\hldef{(num.vec}\hlopt{+}\hlnum{1}\hldef{)}\hlopt{+}\hlnum{2}\hlopt{^}\hldef{(num.vec}\hlopt{-}\hlnum{1}\hldef{)}
\hldef{results}
\end{alltt}
\begin{verbatim}
...
% Use which to find which the index where the results are 40
\begin{knitrout}\scriptsize
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{index.of.result.40} \hlkwb{<-} \hlkwd{which}\hldef{(results}\hlopt{==}\hlnum{40}\hldef{)}
\hldef{index.of.result.40}
\end{alltt}
\begin{verbatim}
## [1] 5
\end{verbatim}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{solution} \hlkwb{<-} \hldef{num.vec[index.of.result.40]}
\hldef{solution}
\end{alltt}
\begin{verbatim}
## [1] 4
...
This approach only works when the solution is an integer because we are evaluating the equation for a strict set of integer values within the num.vec vector. For this example, this method would not find an exact solution since it does not evaluate any non-integer x values.
